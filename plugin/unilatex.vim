...
augroup UNILATEX
	autocmd BufReadPost *.jl call s:LaTeXtoUTF8()
	" autocmd BufWritePre *.tex cal s:UTF8toLaTeX()
	autocmd BufWritePost *.jl call s:LaTeXtoUTF8()
augroup END

for key in keys(s:mapping)
    let s:k = strpart(key, 1)
    execute 'silent! inoreab '.s:k.' <c-r>=<sid>CheckSlash("'.s:k.'")<CR>'.s:mapping[key]
endfor
...
function s:UTF8toLaTeX()
    ...
    for key in keys(s:mapping)
        silent! execute '%s/'.s:mapping[key].'/\'.key.'/eg'
    endfor
