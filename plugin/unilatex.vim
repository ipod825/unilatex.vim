...
function! g:CompleteLatex()
    let l:line = getline('.')
    let l:beg = strridx(l:line, '\')
    if l:beg==-1
        return
    endif
    let l:cur = col('.')-1
    let l:match = strpart(l:line, l:beg+1, l:cur-l:beg+1)
    if has_key(s:mapping, l:match)
        let l:pos = getpos('.')
        execute 's/\\'.l:match.'/'.s:mapping[l:match]
        call setpos('.', l:pos)
    else
        " dealing with characters behind tomatch, e.g. \alpha|)
        let l:match = strpart(l:line, l:beg+1, l:cur-l:beg)
        if has_key(s:mapping, l:match)
            let l:pos = getpos('.')
            execute 's/\\'.l:match.'/'.s:mapping[l:match]
            call setpos('.', l:pos)
